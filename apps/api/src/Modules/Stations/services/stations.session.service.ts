...
export class StationsSessionService {
  ...
  public async selectStation(
    ...
    if (actor.systemRole !== "SuperAdmin") {
      const isAssigned = station.assignedUsers.some(
        (au) => au.userId === actor.id
      )
      if (!isAssigned) {
    ...
    if (station.isIpRestricted) {
      ...
      // Gather and normalize all candidate client IPs
      const candidateIps = [
      ...
      const isWhitelisted =
        ...
        (whitelist.length > 0 &&
          candidateIps.some((clientIp) =>
            whitelist.some((allowedPattern) =>
              isIpInCidr(clientIp, allowedPattern)
            )
          ))
    ...
    if (station.isMacRestricted) {
      const candidateMacs = [
        ...
        .filter((m): m is string =>
          Boolean(m && typeof m === "string" && m.trim())
        )
      ...
      const isMacWhitelisted =
        ...
        (macWhitelist.length > 0 &&
          candidateMacs.some((clientMac) =>
            macWhitelist.some(
              (allowedMac) => allowedMac === clientMac || allowedMac === "*"
            )
          ))
    ...
    const rawIp =
      ...
      undefined
    const ipAddress = rawIp ? normalizeIpAddress(rawIp) : undefined

    // Database transaction: Atomically verify capacity limit and handle station session
    const { sessionRecord, currentOccupancy } = await this.prisma.$transaction(
      async (tx) => {
        ...
        if (existingSession) {
          const activeCount = await tx.stationSession.count({
            where: {
              stationId: station.id,
              isCurrent: true,
              leftAt: null,
          ...
          const updated = await tx.stationSession.update({
          ...
          return { sessionRecord: updated, currentOccupancy: activeCount }
        }
        ...
        const created = await tx.stationSession.create({
          data: {
            stationId: station.id,
            userId: actor.id,
        ...
        return {
          ...
          currentOccupancy: activeConcurrentCount + 1,
        }
      }
    )
    ...
    if (this.cacheManager) {
      await this.cacheManager
        .del([
          ...
          `station:detail:${station.id.toLowerCase()}`,
          "station:stats",
        ])
        .catch(() => {})
    }
  ...
  public async leaveStation(
    ...
    for (const stn of affectedStations) {
      try {
        ...
        realtimeServer.toRoom(
          "stations:overview",
          "station:occupancy_updated",
          {
            stationId: stn.id,
  ...
  public async getActiveSessions(
    ...
    const validSessions = activeSessions.filter(
      (s) => s.station && s.station.isActive && !s.station.deletedAt
    )
    const sanitizedStations = await batchSanitizeAndDecorateStations(
      validSessions.map((s) => s.station),
      actor
    )

    const contexts: ActiveStationContext[] = validSessions.map((sess, idx) => {
      const sanitizedStation = sanitizedStations[idx]!
      const activeProfiles = sanitizedStation.activeProfiles || []
      const activeProfileIds = activeProfiles.map((p) => p.profileId)
