...
export class StationsLookupService {
  ...
  public async createStationType(dto: CreateStationTypeDTO) {
    ...
    if (existing) {
      throw new ConflictError(
        `Station type with code '${formattedCode}' already exists`
      )
    }
  ...
  public async createStationStatus(dto: CreateStationStatusDTO) {
    ...
    if (existing) {
      throw new ConflictError(
        `Station status with code '${formattedCode}' already exists`
      )
    }
    ...
    return this.prisma.stationStatusLookup.create({
      data: {
        ...
        name: dto.name.trim(),
        isOperational:
          dto.isOperational !== undefined ? dto.isOperational : true,
        isMaintenance:
          dto.isMaintenance !== undefined ? dto.isMaintenance : false,
  ...
  public async updateStationStatus(id: string, dto: UpdateStationStatusDTO) {
    ...
    return this.prisma.stationStatusLookup.update({
      ...
      data: {
        name: dto.name !== undefined ? dto.name.trim() : undefined,
        isOperational:
          dto.isOperational !== undefined ? dto.isOperational : undefined,
        isMaintenance:
          dto.isMaintenance !== undefined ? dto.isMaintenance : undefined,
  ...
  public async createStationRole(dto: CreateStationRoleDTO) {
    ...
    if (existing) {
      throw new ConflictError(
        `Station role with code '${formattedCode}' already exists`
      )
    }
    ...
    return this.prisma.stationAssignmentRole.create({
      data: {
        ...
        name: dto.name.trim(),
        canManageProfiles:
          dto.canManageProfiles !== undefined ? dto.canManageProfiles : false,
  ...
  public async updateStationRole(id: string, dto: UpdateStationRoleDTO) {
    ...
    return this.prisma.stationAssignmentRole.update({
      ...
      data: {
        name: dto.name !== undefined ? dto.name.trim() : undefined,
        canManageProfiles:
          dto.canManageProfiles !== undefined
            ? dto.canManageProfiles
            : undefined,
  ...
  public async getStationScopeContext(
    ...
    const managePerm = userPerms["station.manage"]

    if (!managePerm || !managePerm.allowed) {
      return {
    ...
    let authorizedBranches = allBranches
    let authorizedDepartments = allDepartments

    if (managePerm.scope === "Global" || managePerm.scope === "Override") {
      authorizedBranches = allBranches
      authorizedDepartments = allDepartments
    } else if (
      managePerm.scope === "Branch" ||
      managePerm.scope === "OwnBranch"
    ) {
      if (actor.branchId) {
        authorizedBranches = allBranches.filter((b) => b.id === actor.branchId)
        authorizedDepartments = allDepartments.filter(
          (d) => d.branchId === actor.branchId
        )
      } else {
        authorizedBranches = []
        authorizedDepartments = []
      ...
      managePerm.scope === "Department" ||
      managePerm.scope === "OwnDepartment" ||
      managePerm.scope === "Team" ||
      managePerm.scope === "OwnTeam"
    ) {
      ...
      const deptIds = new Set(
        userTeams.map((t) => t.team.departmentId).filter(Boolean)
      )
      authorizedDepartments = allDepartments.filter((d) => deptIds.has(d.id))
      const allowedBranchIds = new Set(
        authorizedDepartments.map((d) => d.branchId).filter(Boolean)
      )
      authorizedBranches = allBranches.filter((b) => allowedBranchIds.has(b.id))
    } else {
      // Fallback for fine-grained custom scopes
    ...
    const isDepartmentRestricted =
      authorizedDepartments.length < allDepartments.length
