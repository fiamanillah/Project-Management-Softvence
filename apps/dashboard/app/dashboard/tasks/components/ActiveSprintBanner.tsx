...
export function ActiveSprintBanner() {
  ...
  // Calculate story point progress and task metrics in a single O(N) pass
  const metrics = React.useMemo(() => {
    ...
    for (const t of tasks) {
      if (t.sprintId === activeSprint.id) {
        totalTasks++;
        const sp = t.storyPoints || 0;
        totalSP += sp;
        if (t.status === "DONE" || t.status === "RESOLVED" || t.status === "PUBLISHED" || t.status === "ASSET_HANDOFF") {
          completedTasks++;
          completedSP += sp;
        }
      }
    ...
    return { totalTasks, completedTasks, totalSP, completedSP, spPercentage, taskPercentage };
  }, [tasks, activeSprint]);

  // Only render on Board and Backlog views when there's an active sprint
  if (!activeSprint || !metrics || (viewMode !== "BOARD" && viewMode !== "BACKLOG")) {
    return null;
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-4 text-xs ml-auto">
          ...
          <div className="flex items-center gap-3 border-l border-border/60 pl-3">
            <div className="flex flex-col gap-1 w-28 sm:w-36">
              <div className="flex justify-between text-[10px] font-medium text-muted-foreground">
                <span>{metrics.completedSP} of {metrics.totalSP} SP</span>
                <span className="text-foreground font-semibold">{metrics.spPercentage}%</span>
              </div>
              <Progress value={metrics.spPercentage} className="h-1.5 bg-muted" />
            </div>

            <div className="hidden sm:flex flex-col text-right text-[10px] text-muted-foreground">
              <span><strong>{metrics.completedTasks}</strong>/{metrics.totalTasks} tasks</span>
              <span className="text-[9px] text-muted-foreground/80">({metrics.taskPercentage}%)</span>
            </div>
          </div>
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-7 text-xs gap-1.5 border-emerald-500/40 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10 font-medium px-2.5 cursor-pointer"
            onClick={() => {
