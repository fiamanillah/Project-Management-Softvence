...
import { Building2, GitFork, Plus } from "lucide-react";
...
export function DivisionGridChart({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 p-4 max-w-7xl mx-auto">
      {roots.map((root) => {
        const isSelected = selectedNode?.department.id === root.department.id;
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2 pb-3 border-b">
              ...
              <Badge variant="outline" className="text-[10px] bg-primary/5 text-primary border-primary/20 shrink-0">
                {root.children.length} Sub-units
              </Badge>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="p-2 rounded-lg bg-muted/40 border">
                <p className="text-[10px] text-muted-foreground">Sub-Depts</p>
                <p className="font-bold text-foreground mt-0.5">{root.children.length}</p>
              </div>
              <div className="p-2 rounded-lg bg-muted/40 border">
                <p className="text-[10px] text-muted-foreground">Roles</p>
                <p className="font-bold text-foreground mt-0.5">{root.department._count?.designations ?? 0}</p>
              </div>
              <div className="p-2 rounded-lg bg-muted/40 border">
                <p className="text-[10px] text-muted-foreground">Teams</p>
                <p className="font-bold text-foreground mt-0.5">{root.department._count?.teams ?? 0}</p>
              </div>
            </div>

            {/* Sub-departments list preview */}
                  ...
                  {root.children.map((child) => (
                    ...
                    >
                      <div className="flex items-center gap-1.5 truncate">
                        <GitFork className="size-3 text-primary shrink-0" />
                        <span className="font-medium truncate">{child.department.name}</span>
                      </div>
                      <span className="font-mono text-[10px] text-muted-foreground shrink-0">
                        {child.department.code}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="pt-2 border-t flex items-center justify-between" onClick={(e) => e.stopPropagation()}>
              <Button
                variant="outline"
                size="sm"
                onClick={() => onAddSubDepartment(root.department)}
                className="h-7 text-xs px-2"
              >
                <Plus className="mr-1 size-3" /> Sub-Unit
              </Button>
              <div className="flex items-center gap-1">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => onEdit(root.department)}
                  className="h-7 text-xs px-2"
                >
                  Edit
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => onAssignManager(root.department)}
                  className="h-7 text-xs px-2"
                >
                  Manager
                </Button>
