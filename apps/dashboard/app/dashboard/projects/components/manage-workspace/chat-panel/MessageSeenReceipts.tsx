        ...
        {visibleSeen.map((r) => {
          ...
          return (
            <Tooltip key={r.userId}>
              <TooltipTrigger
                type="button"
                aria-label={`Seen by ${r.userName} at ${r.seenAt}`}
              ...
              <TooltipContent
                ...
                align="center"
                variant="card"
                className="w-56 p-0 shadow-xl overflow-hidden border border-border/70"
              >
                {/* Header */}
                ...
                <div className="p-2 bg-card/60 flex items-center justify-between text-[10px] text-muted-foreground">
                  ...
                  <span className="font-mono text-[9px]">{r.seenAt}</span>
                </div>
              </TooltipContent>
            </Tooltip>
          );
        ...
        {remainingCount > 0 && (
          <Popover>
            ...
            <PopoverContent
              ...
              className="w-64 p-0 shadow-2xl overflow-hidden border border-border/70"
            >
              {/* Header */}
              ...
              <div className="p-2 space-y-1.5 max-h-52 overflow-y-auto bg-card/40">
                {seenBy.map((r) => {
                  const initials = getUserInitials(r.userName);
                  return (
                    <div
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {r.userDesignation && (
                            <p className="text-[9px] text-muted-foreground truncate">{r.userDesignation}</p>
                          )}
                        </div>
                      </div>
                      <span className="font-mono text-[9px] text-muted-foreground shrink-0">{r.seenAt}</span>
                    </div>
                  );
                })}
              </div>
