...
export function EditStationModal({
  ...
  React.useEffect(() => {
    if (open && station) {
      ...
      setIsIpRestricted(station.isIpRestricted ?? false);
      setIpWhitelist(Array.isArray(station.ipWhitelist) ? [...station.ipWhitelist] : []);
      setIpInput("");
      setIsMacRestricted(station.isMacRestricted ?? false);
      setMacWhitelist(Array.isArray(station.macWhitelist) ? [...station.macWhitelist] : []);
      setMacInput("");
      setIsActive(station.isActive ?? true);
      setFieldErrors({});

      // Fetch server-resolved scope context (Rule FE-3)
      api
