 ...
 */
export function formatSessionStartTime(joinedAt?: string | Date | null): string {
...
export function StationSessionProvider({
  ...
  const { user } = useAuth();
  const [activeSessions, setActiveSessions] = React.useState<ActiveStationContext[]>([]);
  ...
  React.useEffect(() => {
    ...
    const isValid = activeSessions.some((s) => s.station.id === currentStationId);
    if (!isValid) {
  ...
  const refreshSession = React.useCallback(async () => {
    ...
    try {
      const res = await api.get<UserStationSessionsState | ActiveStationContext[] | null>(
      ...
      } else if (rawData && (rawData as any).session && (rawData as any).station) {
  ...
  React.useEffect(() => {
    ...
    const handleStorage = (e: StorageEvent) => {
      if (e.key === FOCUSED_STATION_STORAGE_KEY && e.newValue !== currentStationId) {
  ...
  }, [currentStationId, refreshSession, refreshMyStations]);

  // 2. Real-time WebSocket Event Subscriptions
  React.useEffect(() => {
    ...
    // Join rooms for all active stations
        ...
        prev.map((ctx) =>
          ctx.station.id === data.stationId
            ? {
                ...
                activeProfileIds: (data.activeProfiles || []).map((p) => p.profileId),
              }
            : ctx
        )
      );
  ...
  }, [socket, isConnected, user, activeStationIds, refreshSession, refreshMyStations]);
  ...
  const leaveStation = async (targetStationId?: string) => {
    ...
    try {
      const targetStationName =
        activeSessions.find((s) => s.station.id === stnId)?.station.name || "Workstation";
